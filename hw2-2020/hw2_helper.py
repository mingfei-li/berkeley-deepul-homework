...
def q1_save_results(dset_type, part, fn):
    ...
    show_2d_densities(
        densities, dset_type, fname=f"results/q1_{part}_dset{dset_type}_densities.png"
    )
    show_2d_latents(
        latents, train_labels, f"results/q1_{part}_dset{dset_type}_latents.png"
    )
...
def q2_save_results(fn):
    ...
    save_training_plot(
        train_losses,
        test_losses,
        f"Q2 Dataset Train Plot",
        f"results/q2_train_plot.png",
    )
    show_samples(samples * 255.0 / 2.0, f"results/q2_samples.png")
    show_samples(
        floored_samples * 255.0,
        f"results/q2_flooredsamples.png",
        title="Samples with Flooring",
    )
